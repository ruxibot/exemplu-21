...
window.addEventListener("keydown", (e) => {
  keys[e.key] = true;
  socket.emit("player-update", { keys });
});
window.addEventListener("keyup", (e) => {
  delete keys[e.key];
  socket.emit("player-update", { keys });
});
socket.on("player-update", ({ keys }) => {
  console.log("opponent keys", keys);
  opponentKeys={...keys};
  console.log(opponentKeys);
})
//teren
...
const opponent = {
  ...
  update: function () {
    //console.log(keys);
    if (opponentKeys["ArrowUp"]) {
      this.y -= this.speed;
      if (this.y < teren.top) {
        this.y = teren.top;
      }
    }
    if (opponentKeys["ArrowDown"]) {
      this.y += this.speed;
